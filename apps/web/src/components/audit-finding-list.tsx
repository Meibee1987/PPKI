...
import { findingDispositions, findingsQuery, fixModes, normalizeFindingFilters, severities, type AuditFinding, type AuditFindingPage, type AuditSummary, type FindingDisposition, type FindingFilters, type FixMode, type Severity } from "../lib/audit-contract";
...
import { isFixSelectable, maximumFixPlanItems, mergeFixSelection, selectionReason, toSelectedFinding, type SelectedFixFinding } from "../lib/fix-plan-selection";
...
import { FixPlanWorkflow } from "./fix-plan-workflow";
...
export function AuditFindingList({ identity, summary, refreshSummary }: { identity: CanonicalAuditIdentity; summary: AuditSummary; refreshSummary: () => Promise<void> }) {
  ...
  const [fixSelection, setFixSelection] = useState<SelectedFixFinding[]>([]);
  const [fixSelectionLocked, setFixSelectionLocked] = useState(false);
  ...
  useEffect(() => { setFixSelection([]); setFixSelectionLocked(false); }, [identity.auditId, identity.documentVersionId]);
  ...
  useEffect(() => {
    if (!visible) return;
    const visibleEligibility = new Map(visible.page.items.map(value => [value.id, isFixSelectable(value)]));
    setFixSelection(current => {
      const next = current.filter(value => visibleEligibility.get(value.findingId) !== false);
  ...
  return <section className="panel finding-log" aria-labelledby="finding-log-title" aria-busy={loading}>
    ...
    {visible && <FindingPageView summary={summary} filters={visible.filters} page={visible.page} navigate={navigate} selectFinding={setSelectedFindingId} fixSelection={fixSelection} setFixSelection={setFixSelection} selectionLocked={fixSelectionLocked} />}
    <FixPlanWorkflow identity={identity} selected={fixSelection} locked={fixSelectionLocked} onApproved={() => setFixSelectionLocked(true)} onClear={() => setFixSelection([])} onAuthoritativeRefresh={() => setReload(value => value + 1)} />
...
function FindingPageView({ summary, filters, page, navigate, selectFinding, fixSelection, setFixSelection, selectionLocked }: { summary: AuditSummary; filters: FindingFilters; page: AuditFindingPage; navigate: (filters: FindingFilters) => void; selectFinding: (findingId: string) => void; fixSelection: SelectedFixFinding[]; setFixSelection: (value: SelectedFixFinding[]) => void; selectionLocked: boolean }) {
  ...
  const selectedIds = new Set(fixSelection.map(value => value.findingId));
  const selectable = page.items.map(toSelectedFinding).filter((value): value is SelectedFixFinding => value !== null);
  const allPageSelected = selectable.length > 0 && selectable.every(value => selectedIds.has(value.findingId));
  const toggle = (finding: AuditFinding) => {
    const value = toSelectedFinding(finding);
    if (!value || selectionLocked) return;
    setFixSelection(selectedIds.has(value.findingId) ? fixSelection.filter(item => item.findingId !== value.findingId) : mergeFixSelection(fixSelection, [value]));
  };
  const togglePage = () => {
    if (selectionLocked) return;
    const pageIds = new Set(selectable.map(value => value.findingId));
    setFixSelection(allPageSelected ? fixSelection.filter(value => !pageIds.has(value.findingId)) : mergeFixSelection(fixSelection, selectable));
  };
  return <>
    ...
    <div className="fix-selection-toolbar"><div><strong>{fixSelection.length} dari maksimum {maximumFixPlanItems} temuan dipilih</strong><p>Pilihan tetap tersimpan saat berpindah halaman atau mengubah filter.</p></div><button className="button secondary" type="button" disabled={!selectable.length || selectionLocked} onClick={togglePage}>{allPageSelected ? "Batalkan pilihan aman di halaman ini" : "Pilih semua perbaikan aman di halaman ini"}</button></div>
    <ol className="finding-log-list" start={range.start}>{page.items.map(item => <li key={item.id}><article>
      <header><div>{isFixSelectable(item) && <input className="fix-selection-checkbox" type="checkbox" aria-label={`Pilih ${item.ruleCode} untuk rencana perbaikan`} checked={selectedIds.has(item.id)} disabled={selectionLocked} onChange={() => toggle(item)} />}<span className={`severity severity-${item.severity.toLowerCase()}`}>{item.severity}</span><strong>{item.ruleCode}</strong><span className="domain-label">{item.domain}</span></div><DocumentPageLocation versionId={page.documentVersionId} value={item.pageLocation} /></header>
      ...
      <p className="fix-mode-copy">{selectionReason(item)}</p>
