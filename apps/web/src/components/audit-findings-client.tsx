...
import { formatTimestamp, pageRange, scorePresentation } from "../lib/findings-presentation";
...
function FindingsSection({ auditId, documentVersionId, filters, page, loading, error, retry, navigate }: { auditId: string; documentVersionId: string; filters: FindingFilters; page?: AuditFindingPage; loading: boolean; error: string; retry: () => void; navigate: (filters: FindingFilters) => void }) {
  ...
  return <section className="panel findings-panel" aria-labelledby="findings-title"><div className="section-heading"><div><h2 id="findings-title">Daftar temuan</h2><p>Menampilkan {range.start}–{range.end} dari {page.totalCount}. Urutan berasal dari backend.</p></div></div><ol className="findings-list" start={range.start}>{page.items.map(item => <li key={item.id}><article className="finding-item"><header><div className="finding-identifiers"><span className={`severity severity-${item.severity.toLowerCase()}`}>{item.severity}</span><strong className="rule-code" title={item.ruleCode}>{item.ruleCode}</strong><span className="domain-label">{item.domain}</span></div><div><FindingLocation value={item.location} /><DocumentPageLocation versionId={documentVersionId} value={item.pageLocation} /></div></header><h3>{item.element}</h3><p className="finding-reason">{item.reasonCode}</p><div className="comparison"><FindingPayload label="Aktual" value={item.actual} /><FindingPayload label="Diharapkan" value={item.expected} /></div><footer><span>Mode: <strong>{item.fixMode}</strong></span><span>Aksi tersedia: <strong>{item.actionAvailability === "None" ? "Tidak ada" : item.actionAvailability}</strong></span><Link className="button secondary" href={`/audits/${encodeURIComponent(auditId)}/findings/${encodeURIComponent(item.id)}?${query}`}>Lihat detail</Link></footer></article></li>)}</ol><nav className="pagination" aria-label="Navigasi halaman temuan"><button className="button secondary" disabled={page.page <= 1} onClick={() => navigate({ ...filters, page: page.page - 1 })}>Sebelumnya</button><span>Halaman {page.page} dari {range.totalPages}</span><button className="button secondary" disabled={page.page >= range.totalPages} onClick={() => navigate({ ...filters, page: page.page + 1 })}>Berikutnya</button></nav></section>;
}
