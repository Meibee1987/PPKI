...
insert into public.documents(id,owner_user_id,document_type_id,title,current_version_no) values('${ids.document}','${ownerId}','${ids.documentType}','Synthetic resolution smoke',1) on conflict(id) do nothing;
...
update public.fix_execution_jobs set state='Processing',claim_token='${ids.execution}',attempt_count=1,started_at=now(),lease_expires_at=now()+interval '10 minutes' where id='${ids.execution}' and state='Queued';
update public.fix_execution_jobs set state='Completed',result_document_version_id='${ids.resultVersion}',result_sha256='${"b".repeat(64)}',result_object_size=1,completed_operation_count=5,claim_token=null,lease_expires_at=null,completed_at=now() where id='${ids.execution}' and state='Processing';
begin;
...
async function main() {
  ...
  try {
    ...
    await expectConflict(apiUrl, environment, owner.token, container, "non-completed-execution-is-safe", `update public.fix_execution_jobs set state='Queued',result_object_size=null,object_created_by_attempt=null where id='${ids.execution}'`, `update public.fix_execution_jobs set state='Completed',result_object_size=1 where id='${ids.execution}'`, "resolution-execution-not-completed");
    ...
    await expectConflict(apiUrl, environment, owner.token, container, "lineage-mismatch-is-safe", `update public.document_versions set sha256='${"e".repeat(64)}' where id='${ids.resultVersion}'`, `update public.document_versions set sha256='${"b".repeat(64)}' where id='${ids.resultVersion}'`, "resolution-lineage-mismatch");
