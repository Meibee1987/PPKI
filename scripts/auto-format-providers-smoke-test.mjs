...
async function main() {
  console.log("SUITE auto-format-providers-local-production-e2e"); let temporary;
  try {
    await run("docker", ["info", "--format", "{{.ServerVersion}}"], { timeoutMs: 30_000 });
    const environment = await getSupabaseEnvironment(process.cwd()); const container = await databaseContainer();
